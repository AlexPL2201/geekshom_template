window.onload = function () {
	...
	//1 метод обновления стоимости и кол-ва
	...
	//2 метод обновления стоимости и кол-ва
        ...
        if (orderitem_product_pk) {
            $.ajax({
                ...
                success: function (data) {
                   ...
                   /* let count_html = '<input type="number" name="orderitems-' + orderitem_num + 
                    '-quantity" value="1" min="0" class="form-control" id="id_orderitems-' + orderitem_num + '-quantity">'*/

                    let current_tr = $('.order_form table').find('tr:eq('+(orderitem_num+1)+')');
                    current_tr.find('td:eq(2)').html(price_html);
                    
                    /*current_tr.find('td:eq(1)').html(count_html);*/

                    orderSummaryUpdate(price_arr[orderitem_num], 1);
                }
            })
